...
app.get('/', (req,res)=>res.send('Dukaandaar AI Live v3.1 Fixed - 93 lines'));
...
app.get('/shop', (req,res)=>{
  res.send('<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Shop</title><style>body{font-family:system-ui;background:#f6f6f6;margin:0}header{background:#0a7c42;color:#fff;padding:14px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:12px}.card{background:#fff;border-radius:12px;padding:10px}.price{color:#0a7c42;font-weight:700}button{background:#0a7c42;color:#fff;border:none;padding:8px;border-radius:8px;width:100%;margin-top:6px}</style></head><body><header><h3>Dukaandaar AI Shop - Mohone</h3><input id="s" placeholder="Search Atta, Bucket..." style="width:100%;padding:8px;border-radius:8px;border:none;margin-top:8px"></header><div class="grid" id="g"></div><div style="position:fixed;bottom:0;left:0;right:0;background:#fff;padding:10px;display:flex;justify-content:space-between"><b id="c">0 items</b><span>Rs <span id="t">0</span></span></div><script>let products=[],cart=[];async function load(){try{let r=await fetch("/api/products");products=await r.json();if(!products.length)throw"";}catch(e){products=[{id:1,name:"Aashirvaad Atta 5kg",price:250},{id:2,name:"Aata 1kg Local",price:55},{id:3,name:"Bucket Small",price:2230},{id:4,name:"Tata Salt",price:30}];}render();}function render(){let q=document.getElementById("s").value.toLowerCase();let g=document.getElementById("g");g.innerHTML="";products.filter(p=>!q||p.name.toLowerCase().includes(q)||(q.includes("atta")&&p.name.toLowerCase().includes("aata"))).forEach(p=>{g.innerHTML+="<div class=card><div>"+p.name+"</div><div class=price>Rs "+p.price+"</div><button onclick=\"cart.push({id:"+p.id+",name:\'"+p.name+"\',price:"+p.price+",qty:1});document.getElementById('c').innerText=cart.length+' items';let tot=0;cart.forEach(c=>tot+=c.price);document.getElementById('t').innerText=tot\">Add to Cart</button></div>";});}document.getElementById("s").addEventListener("input",render);load();</script></body></html>');
});
